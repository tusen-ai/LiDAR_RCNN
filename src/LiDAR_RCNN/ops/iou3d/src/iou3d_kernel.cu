...
__global__ void boxes_overlap_kernel(const int num, const float *boxes_a, const float *boxes_b, float *ans_overlap){
...
void boxesoverlapLauncher(const int num, const float *boxes_a, const float *boxes_b, float *ans_overlap){
    ...
    boxes_overlap_kernel<<<DIVUP(num, THREADS_PER_BLOCK), THREADS_PER_BLOCK>>>(num, boxes_a, boxes_b, ans_overlap);
#ifdef DEBUG
